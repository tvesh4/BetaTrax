...
- **Developer Metrics:** System calculates a developer's effectiveness using the ratio of (number of reopened cases) / (number of fixed cases). It returns any of the string `Insufficient data` (if number of fixed cases < 20), `Good` (ratio < 1/32), `Fair` (ratio < 1/8), or `Poor` (else case). Exposed at `GET /api/metric/<dev_id>/`. A developer's fixed count and reopened count can also be changed in Django admin site. 
- **Automated Testing:** Nineteen tests under `BTAPI/tests.py` — thirteen endpoint smokes (one happy-path per endpoint method satisfying §38, plus three extra cases that pin (a) the `CLOSED` terminal-state union, (b) Owner-only `severity`/`priority`/`dev` mutations, and (c) the negative role-gate path) plus six classifier unit tests that produce 100% statement and branch coverage on `BTAPI/metrics.py` (the §25 grading hook). Tests run under `BTConfig.settings_test`, a SQLite-backed module that strips `django_tenants` for speed; tenant isolation is therefore not exercised by the suite (see Limitations).
